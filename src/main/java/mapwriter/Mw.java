...
import mapwriter.map.EntityMarkerManager;
...
public class Mw {
	...
	public EntityMarkerManager entityMarkerManager = null;
	...
	public void addEntityMarker(Entity entity, int displayTime)
	{
		if(this.entityMarkerManager!=null)
		{
			this.entityMarkerManager.put(this.mc.thePlayer, entity, displayTime);
		}
	}
	...
	public void impactToMapPos(MapView mapView, int x, int y, int z) {
		if(this.mc.thePlayer.getCurrentEquippedItem() != null) {
			if (this.mc.thePlayer.getCurrentEquippedItem().getItem() instanceof ItemGun) {
				//double scale = mapView.getDimensionScaling(this.playerDimension);
				//this.teleportTo((int) (x / scale), y, (int) (z / scale));
				ItemGun itemGun = (ItemGun) this.mc.thePlayer.getCurrentEquippedItem().getItem();
				double scale = mapView.getDimensionScaling(this.playerDimension);
				itemGun.impactX = (int) (x / scale);
				itemGun.impactY = (int) (y / scale);
				itemGun.impactZ = (int) (z / scale);
				//FlansMod.getPacketHandler().sendToAllAround(new PacketFlashBang(10), this.mc.thePlayer.posX, this.mc.thePlayer.posY, this.mc.thePlayer.posZ, 10, this.mc.thePlayer.dimension);
				FlansMod.getPacketHandler().sendToServer(new PacketImpactPoint((int) (x / scale), (int) (y / scale), (int) (z / scale), this.mc.thePlayer.getEntityId()));
				this.mc.displayGuiScreen(null);
				//itemGun.sendPosToServer = true;
				//itemGun.sendPosToServerFunction(x, y, z);
	
			} else {
				MwUtil.printBoth("cannot set impact point");
			}
		}
	}
	...
	public void load() {
		...
		this.entityMarkerManager = new EntityMarkerManager();
	...
	public void close() {
		...
		if (this.ready) {
			...
			this.entityMarkerManager.clear();
	...
	public void onTick() {
		...
		if (this.ready && (this.mc.thePlayer != null)) {
			...
			this.entityMarkerManager.onTick(this.mc.thePlayer);
